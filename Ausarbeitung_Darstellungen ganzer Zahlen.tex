...
\section{b-adische Darstellung natürlicher Zahlen}
  ...
  \begin{satz}[b-adische Darstellung] \label{satz:b-adische_Darstellung}
    ...
    \end{equation}
    erfüllen.
    $l$ und $z_i$ sind dabei eindeutig bestimmt. Dann heißt $z_{l-1}\dots z_0$ die \textbf{b\nbdash adische Darstellung} von n und man schreibt auch $n=(z_{l-1} \dots z_0)_b$. \\
    Es gilt: 
    \begin{equation}
  ...
  Für den Beweis des Satzes benötigen wir das folgende Lemma:
    ...
    In dieser Ausarbeitung werden die Auf- und Abrundungsfunktion (auch Gaußklammer genannt) an einigen Stellen verwendet.
    ...
    \begin{proof}[Beweis von Satz~\ref{satz:b-adische_Darstellung}] 
      ...
      Weiter gilt:
      \begin{align*}
        &n = \sum_{i=0}^{l-1} z_ib^i \underbrace{\leq}_{z_i \leq b-1} \sum_{i=0}^{l-1} (b-1)b^i = (b-1) \underbrace{\sum_{i=0}^{l-1} b^i}_{\mathclap{\text{geometrische Reihe}}} = (b-1) \frac{b^l-1}{b-1} = b^l-1 \\
        &\Rightarrow n \leq b^l -1 \\
        &\Rightarrow n < b^l.
      \end{align*}
      Es gilt also: $b^{l-1} \leq n < b^l$. 
      ...
      Wir beweisen die Existenz der $b$-adischen Darstellung durch vollständige Induktion über die Länge der Darstellung $l(n) \coloneq 1+ \lfloor \log_b(n) \rfloor$. 
      ...
      \textbf{Induktionsschritt: $l-1 \to l$} 

      Sei $n\in\N$ mit $l(n) = l \geq 2$, setze $n' \coloneq \left\lfloor \displaystyle\frac{n}{b} \right\rfloor$.
      ...
      \[n' = \sum_{i=0}^{l' -1} z'_i b^i\] 
      mit $z'_i \in \{0,\dots,b-1\} \text{ und } z'_{l'-1} \neq 0$. \\
      Setze $z_0 \coloneq n \bmod b \in \{0, \dots, b-1 \}$. Dann gilt:
      \[n = b \cdot \left\lfloor \frac{n}{b} \right\rfloor + (n \bmod b) = b \cdot n' + z_0\]
      Einsetzen von $n' = \displaystyle\sum_{i=0}^{l'-1}z'_i b^i$ nach Induktionsvoraussetzung ergibt
      \begin{align*}
      ...
      \end{align*}
      Da $l' = l-1$, setze $z_j \coloneq z'_{j-1}$ für $j \in \{1,\dots,l-1\}$ und wir erhalten die gewünschte Darstellung
      \begin{equation*}
...
\section{b-Komplementdarstellung ganzer Zahlen}
  ...
  \subsection{b-Komplementdarstellung - Einführung und Beispiel}
    ...
    Man nimmt jede negative Zahl $z\in \{-2^{l-1},\dots,-1\}$ mit $z+2^l$, wobei $l$ hier für die Anzahl der verwendeten Bits steht. 
    ...
    \begin{beispiel} \label{bsp:Komplementdarstellung_Erklärung}
      Bei $l=4$ verfügbaren Bits wird die Zahl $z=-3$ wie folgt dargestellt:
      \begin{align*}
        z+2^l = -3+16 = 13
      \end{align*}
...
\subsection{b-Komplementdarstellung - allgemeine Formulierung}
  ...
  \begin{definition}[$l$ stelliges $b$-Komplement] \label{def:b-Komplement}
    Seien $l,b \in \N$, mit $b\geq 2$, sei $n \in \{0, \dots, b^l-1\}$.
    Dann ist 
    \[K^l_b(n) \coloneq (b^l-n) \bmod b^l\]
    das \textbf{$l$-stellige $b$-Komplement} von $n$.
  ...
  \begin{lemma} \label{lem:b-Komplement_berechnen}
    Seien $l,b \in \N$, mit $b\geq 2$. Sei $n=\displaystyle\sum_{i=0}^{l-1} z_ib^i$ mit $z_i \in \{0, \dots, b-1\}$ für $i = 0, \dots, l-1$.
    Dann gilt:
    ...
    \begin{proof}
      \begin{description}
        ...
        \item[\ref{lem:b-Komplement_berechnen_Berechnung}] Sei nun $n \in \{0,\dots,b^l-2\}$.
          Es gilt nach Definition~\ref{def:b-Komplement} für $m \in \{0, \dots, b^l-1\}:$ 
          \[K^b_l(m)=(b^l-m) \bmod b^l.\]
          Da $0 \leq n \leq b^l -2$ ist, folgt für $m = n+1$:
          \[1 \leq n+1 \leq b^l-1.\]
          Das heißt dann gilt für $(b^l - (n+1))$ 
          \[1 \leq b^l-(n+1) \leq b^l -1,\] 
          liegt also in $\{1, \dots, b^l-1\}$.\\
          Es folgt, dass die Modulo-Operation trivial ist:
          \[K^b_l(n+1)=(b^l-(n+1)) \bmod b^l =b^l-1-n.\]
          Wegen der geometrischen Reihe $b^l-1 = \displaystyle\sum_{i=1}^{l-1} (b-1)b^i$ gilt:
          \begin{align*}
            K^b_l(n+1) &= b^l-1-n \\
            &= \left(\sum_{i=0}^{l-1}(b-1)b^i\right) -n \\
            &\underbrace{=}_{\mathclap{\text{Satz \ref{satz:b-adische_Darstellung}}}} \left(\sum_{i=0}^{l-1}(b-1)b^i\right) - \underbrace{\left(\sum_{i=0}^{l-1} z_ib^i\right)}_{\mathclap{b\text{-adisch}}} \\
  ...
  \begin{beispiel}
    \begin{enumerate}
      \item Sei $b=10, l=4$. Wir berechnen $K_4^{10}(4809)$.
        \begin{enumerate}
...
\subsection{l-stellige b-Komplementdarstellung}
  Möglich gemacht wird diese einheitliche Darstellung mit der folgenden Definition:

  \begin{definition}[l-stellige b-Komplementdarstellung] \label{def:l-stellige_b-Komplementdarstellung}
    ...
    Man erhält die \textbf{$l$-stellige $b$-Komple-mentdarstellung} von $n$, indem man die \hyperref[satz:b-adische_Darstellung]{$b$-adische Darstellung} von $n$, falls $n \geq 0$,
    ...
    \begin{proof}
      \begin{description}
        \item[Bijektivität] Wir betrachten Definitions- und Zielbereich von $f$.
          ...
          \noindent Da $f$ injektiv ist, und $|Z| = b^l = |\{0,\dots,b^l-1\}|$, also Definitions- und Zielbereich die gleiche Kardinalität haben, ist $f$ bijektiv.
      \end{description}
